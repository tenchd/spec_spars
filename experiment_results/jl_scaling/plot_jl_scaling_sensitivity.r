...
pdf("jl_scaling_sensitivity.pdf", width = 14, height = 10)
...
ggplot(data, aes(x = jl_scaling_factor, y = sparsification_rate * 100, color = sketch_type)) + 
geom_point() +
geom_line() +
scale_x_continuous(name ="JL scaling factor", trans = "log2") +
scale_y_continuous(name = "Sparsification Rate (% edges remaining)") +
scale_color_discrete(name = "Sketch Type") +
ggtitle("Sparsification Rate as a function of JL scaling factor") +
facet_wrap(~dataset)

ggplot(data, aes(x = jl_scaling_factor, color = sketch_type, fill = sketch_type)) + 
...
ggplot(data, aes(x = jl_scaling_factor, y = (evim_time + jl_time + solve_time + diff_norm_time + reweight_time), color = sketch_type)) +
geom_line() +
geom_point() +
scale_x_continuous(name ="JL scaling factor", trans = "log2") +
...
scale_color_discrete(name = "Sketch Type") +
ggtitle("Runtime as a function of JL scaling factor") +
facet_wrap(~dataset)
...
data %>% 
  ...
  walk(function(d_sub) {
    ...
    # ---- Sparsification Rate ---------------------------------
    p_rate <- ggplot(d_sub, aes(x = jl_scaling_factor, y = sparsification_rate * 100, color = sketch_type)) + 
        geom_point(size = 2) +
    ...
    # ---- Error rate -----------------------------
    p_error1 <- ggplot(d_sub, aes(x = jl_scaling_factor, color = sketch_type, fill = sketch_type)) + 
    ...
    p_error2 <- ggplot(d_sub, aes(x = jl_scaling_factor, color = sketch_type, fill = sketch_type)) + 
        geom_bar(aes(y = (upper_bound_violations + lower_bound_violations)), stat = "identity", alpha = 0.5, position = "dodge") +
        scale_x_continuous(name ="JL scaling factor", trans = "log2") +
    ...
    # ---- Runtime --------------------------------------------------------------
    p_time <- ggplot(d_sub, aes(x = jl_scaling_factor, y = (evim_time + jl_time + solve_time + diff_norm_time + reweight_time), color = sketch_type)) +
